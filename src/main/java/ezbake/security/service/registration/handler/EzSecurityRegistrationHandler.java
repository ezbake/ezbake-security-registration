...
import ezbake.security.persistence.exception.*;
...
public class EzSecurityRegistrationHandler extends EzBakeBaseThriftService implements EzSecurityRegistration.Iface {
    ...
    public EzSecurityRegistrationHandler (Properties properties) throws AccumuloException, AccumuloSecurityException, PKeyCryptoException, IOException {
        ...
        this.passphrase = calculatePassphrase(crypto);
        this.encryptPk = Boolean.parseBoolean(properties.getProperty(SECURITY_ENCRYPT));
    }

    @Override
    public TProcessor getThriftProcessor() {
        EzSecurityRegistration.Processor<EzSecurityRegistrationHandler> processor = null;

        try {
            processor = new EzSecurityRegistration.Processor<>(new EzSecurityRegistrationHandler(getConfigurationProperties()));
        } catch (AccumuloException e) {
            log.error("Accumulo failure creating registration manager {}", e);
        } catch (AccumuloSecurityException e) {
            log.error("Accumulo security exception failure creating registration manager {}", e);
        } catch (PKeyCryptoException e) {
            log.error("Error Signing Key {}", e);
        } catch (IOException e) {
            log.error("Error Getting Private Key Resource {}", e);
        }
        return processor;
    }
    ...
    public String registerApp(EzSecurityToken ezToken, String appName, String level,
        ...
        try {
            ...
            model.setVisibilityLevel(level);
            model.setVisibility(authorizations);
	...
	private String calculatePassphrase(PKeyCrypto crypto) throws PKeyCryptoException{
	    ...
	    String encodedPassphrase = Base64.encodeBase64String(signedData);
